...
const usable=[...byGtin.values()].filter(v=>v.length===1).map(v=>v[0]);
...
console.log('Barcode lookup/Collector synchronized: '+usable.length+' usable format-valid associations; source verification is separate.');
